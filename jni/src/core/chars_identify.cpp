...
/*! \namespace easypr
        ...
        */
namespace easypr {
ofstream fout("/sdcard/easypr/Debug.txt",ios::app);
...
int CCharsIdentify::classify(Mat f, bool isChinses, bool isSpeci) {
  ...
  fout<<"f.size: "<<f.size()<<std::endl;
  fout<<"output.size: "<<output.size()<<std::endl;
  fout<<"call predict"<<std::endl;
  ann.predict(f, output);  //使用ann对字符做判断

  fout<<"isChinses: "<<isChinses<<std::endl;
  if (!isChinses)  //对中文字符的判断
  {
    fout<<"isSpeci: "<<isSpeci<<std::endl;
	if (isSpeci) {
      result = 0;
      float maxVal = -2;
...
string CCharsIdentify::charsIdentify(Mat input, bool isChinese, bool isSpeci) {
  fout<<"call features"<<std::endl;
  Mat f = features(input, m_predictSize);

  string result = "";
  fout<<"call classify"<<std::endl;
  int index = classify(f, isChinese, isSpeci);  //使用ann来判别那个字符
  fout<<"classify: "<<index<<std::endl;
